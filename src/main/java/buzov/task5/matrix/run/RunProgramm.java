package buzov.task5.matrix.run;

import buzov.task5.matrix.Matrix;
import buzov.task5.matrix.MatrixDouble;
import buzov.task5.matrix.data.customer.CustomerDAOInterface;
import buzov.task5.matrix.data.customer.CustomerDaoHistory;
import buzov.task5.matrix.data.dao.DAODataBase;
        ...
        try {
            ...
            while (worker) {
                try {
                    ...
                    matrixTable = factory.getCustomerDAO();
                    ...
                    System.out.println("inserts two matrixes into a database");
                    matrixTable.insert(matrixA);//, 1);
                    matrixTable.insert(matrixB);//, 2);

                    //reads out matrixes from the database
                    System.out.println("reads out matrixes from the database");
                    matrixA = (Matrix) matrixTable.select(matrixA.getId());
                    matrixB = (Matrix) matrixTable.select(matrixB.getId());
                    ...
                    CustomerDaoHistory cdh = factory.getCustomerHistoryDAO();
                    
                    System.out.println(cdh.selectHistory());
